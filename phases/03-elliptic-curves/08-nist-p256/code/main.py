...
def lift_x_p256(x: int, y_parity: int) -> Point:
    if y_parity not in {0, 1}:
        raise ValueError("y_parity must be 0 or 1")
    p = P256_P
    x %= p
    rhs = (pow(x, 3, p) + (P256_A * x) + P256_B) % p
...
def main() -> None:
    ...
    print("p-256 public key demo (secp256r1)")
    print(f"k = {k}")
    print("Q = k·G")
    print("x =", hex(pub.x))
    print("y =", hex(pub.y))
    print("compressed =", serialize_compressed(pub).hex())
    print("uncompressed =", serialize_uncompressed(pub).hex()[:12] + "...")
